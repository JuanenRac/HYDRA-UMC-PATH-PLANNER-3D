...
pub enum PathSafetyIssue {
    ...
    /// `Obstacle`'s own effective collision radius negative and
    /// silently report a crossing segment as `status: "safe"`.
    InvalidScenario {
        reason: crate::semantics::InvalidScenarioReason,
    },
}
...
pub fn validate_path(
    ...
    if let Err(reason) = crate::semantics::validate_robot_radius(robot_radius) {
        return vec![PathSafetyIssue::InvalidScenario { reason }];
    }
    if let Err(reason) = crate::semantics::validate_obstacles(obstacles) {
        return vec![PathSafetyIssue::InvalidScenario { reason }];
    }
    if let Err(reason) = crate::semantics::validate_workspace(workspace) {
        return vec![PathSafetyIssue::InvalidScenario { reason }];
    }
...
mod tests {
    ...
    // passing directly through the obstacle's own center reported
    // `status: "safe"` - a real, silent defeat of collision detection,
    // not merely "less safe".
    #[test]
    fn a_negative_robot_radius_is_rejected_instead_of_silently_reporting_safe() {
        let obstacles = vec![crate::obstacle::Obstacle::new(
            Vec3::new(0.0, 0.0, 0.0),
            1.0,
        )];
        let workspace = crate::rrt::Workspace {
            min: Vec3::new(-10.0, -10.0, -10.0),
            max: Vec3::new(10.0, 10.0, 10.0),
        };
        let path = vec![Vec3::new(-5.0, 0.0, 0.0), Vec3::new(5.0, 0.0, 0.0)];

        let issues = validate_path(&path, &obstacles, &workspace, -2.0);

        assert_eq!(
            issues,
            vec![PathSafetyIssue::InvalidScenario {
                reason: crate::semantics::InvalidScenarioReason::NegativeRobotRadius
            }]
        );
    }
